...
package com.yogurt3d.core.managers
{
	...
	public class SceneTreeManager
	{
		...
		public static function initIntersectedLightByCamera(_scene:Scene3D, _activeCamera:Camera3D):void
		{
			...
			{				
				...
				for ( k = 0; k < lights.length; k++) 
				{
					...
					{
						getSceneRenderableSetLight(_scene, _light, k, _light.YOGURT3D_INTERNAL::m_renderQueue );
						SceneTreeManager.s_intersectedLightsByCamera[_activeCamera].push(_light);
					}
				}
			}
		...
		public static function getSceneRenderableSet(_scene:Scene3D, _camera:Camera3D, m_renderQueue:RenderQueue):void
		{
			IRenderableManager(s_sceneTreeManagerByScene[ _scene ]).getSceneRenderableSet( _scene, _camera, m_renderQueue );
		}
		
		
		public static function getSceneRenderableSetLight(_scene:Scene3D, _light:Light, _lightIndex:int, _renderQueue:RenderQueue):void
		{
			IRenderableManager(s_sceneTreeManagerByScene[ _scene ]).getSceneRenderableSetLight( _scene, _light, _lightIndex,_renderQueue );
