...
package com.yogurt3d.presets.scene.quad
{
	...
	public class QuadTree
	{
		...
		public function visibilityProcess( camera:Camera3D,_renderQueue:RenderQueue):void{
			_visibilityProcess(camera.frustum, m_root, true,_renderQueue);
		}
				
		public function visibilityProcessLight( light:Light, lightIndex:int, _scene:Scene3D,_renderQueue:RenderQueue):void{
			_visibilityProcessLight(light.frustum, m_root, true, lightIndex, _scene,_renderQueue);
		}
			
		// recursive
		private function _visibilityProcess( frustum:Frustum, node:QuadNode, bTestChildren:Boolean,_renderQueue:RenderQueue) :void
		{		
			var i:int;
					...
					case (Frustum.IN):
						...
						if(lena > 0)
						{
							for(i = 0; i < lena; i++)
							{
								...
								_renderQueue.addRenderable(quadrantSceneObject);
							}
						}
						bTestChildren = false;
						break;
			...
			if(node.m_numNodes)
			{
				if ( (childNode = nodesOfNode[0]) != null && childNode.m_sumChildren)
					_visibilityProcess( frustum, childNode, bTestChildren, _renderQueue);
		...
		// recursive
		private function _visibilityProcessLight( frustum:Frustum, node:QuadNode, bTestChildren:Boolean, lightIndex:int, _scene:Scene3D, _renderQueue:RenderQueue) :void
		{		
			var i:int;
			...
			{
				if ( (childNode = nodesOfNode[0]) != null && childNode.m_sumChildren)
					_visibilityProcessLight( frustum, childNode, bTestChildren, lightIndex, _scene, _renderQueue);
